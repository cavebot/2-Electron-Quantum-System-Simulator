...
CONTAINS
  ...
  !
  !
  SUBROUTINE init_symmetry_cv(this,li)
    !
    IMPLICIT NONE
    !
    TYPE(symmetry_ls2e), INTENT(inout) :: this
    ...
    !    loop_L_symmetries: DO li = 0, lmax
     ...
     !f90
       ...
       ! read energies/coefficients from CI calculation
        ...
        !        ALLOCATE( this%pv(1:n2e,1:nhx))     !'n' energy eigenstate index, 'c' --> configuration index
        ! take the probability
        !DO ic = 1, nhx !SIZE(this%cv,dim=2)
        !   DO ie = 1, n2e !SIZE(this%cv,dim=1)
        !      this%pv(ie,ic) = this%cv(ie,ic)**2 
        !   ENDDO
        !ENDDO
   ...
   END SUBROUTINE init_symmetry_cv
...
PROGRAM tdse_pop_sdi
  ...
  USE ncFile_new
  !
  USE atom_2e, ONLY: symmetry_ls2e, init_symmetry_cv
  ...
  TYPE(symmetry_ls2e), ALLOCATABLE, DIMENSION(:) :: w2e_tst
...
!!%  CLOSE(ncoe)
  ...
  ENDDO
     ...
     ALLOCATE(   w2e_tst(0:lmax) )
     !     DO l = 0, lmax
        !CALL init_symmetry_ct( w2e(l),l)
        !        CALL init_symmetry_cv(w2e(l),l)
     !     ENDDO
     !     STOP
...
!!%
     ...
     !f90
        ...
        !        ALLOCATE( w2e(l)%pv(1:n2e,1:nhx))     !'n' energy eigenstate index, 'c' --> configuration index
        ! take the probability
        !DO ic = 1, nhx
        !   DO ie = 1, n2e
        !      w2e(l)%pv(ie,ic) = w2e(l)%cv(ie,ic)**2 
        !   ENDDO
        !ENDDO
  ...
  partial_probability_Lv:DO  l = 0, lmax
     ...
     population_of_cfg_a:  DO ic = 1, w2e(l)%ncf  ! cfg index  
        ...
        zsum = 0.0_dpk
           DO ie = ie_min, ie_max
              zsum = zsum + w2e(l)%cv(ie,ic) * w2e(l)%ct(ie)
           ENDDO
        
        w2e(l)%pvt_0(ic) = ABS(zsum)**2
  ...
  angular_symmetry_2e_zero_order:DO l = 0, lmax
     ...
     sum_all_configurations: DO ic = 1, w2e(l)%ncf
        ...
        pick_channels:IF( (e1>0).AND.(e2>0) )  THEN
           ...
           IF((le1.EQ.3).AND.(le2.EQ.3)) sum_d_ff = sum_d_ff + pe_d
