...
MODULE utils
  ...
  PUBLIC asciifile, datafile, check, print_mx, banded_mat_v_mul, dfile,afile
  ...
  SUBROUTINE asciifile(n, l, filename, type)
    ...
    IF(L.GE.0) THEN
       ...
       IF(PRESENT(TYPE)) THEN
          ...
          ASCII_FILE = "dat/"//FILENAME//TRIM(ADJUSTL(SL))//"." &
                           //TRIM(ADJUSTL(TYPE))               &
                           //".dat"
       ELSE
