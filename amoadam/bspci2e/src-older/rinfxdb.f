...
C........
      ...
      if(id.eq.0) then 
         ...
         print*," call sine grid  r_min, rmax, dr = ",r(2),r(no),h
      else if(id.eq.1) then
         call rlin(r,dr,rmax,h,no)
         print*," call linear grid   r_min, rmax, dr = ",r(1),r(no),h
         ...
         print*," acceptable values for  idbsb = 1, linear-like grid"
         print*,"                        idbsb = 2,    exp-like grid"
         stop
      endif
...
C# this is dR = (r/dr)*dx not dr      
      do  j=1,no
        dr(j) = dble(nrp) * (dlog(r(j)/ri))**(nrp-1)
      enddo
