...
!!%     !     USE PRECISION, ONLY:DPK
!!%     IMPLICIT NONE
!!%     INTEGER n, k, jderiv, mflag, i, km1
!!%     INTEGER j, jcmin, imk, jcmax, ilo, jj, nmi, jc, kmj
!!%     REAL(DPK) x, fkmj
!!%     REAL(DPK), DIMENSION(ns+nk) :: t
!!%     REAL(DPK),DIMENSION(nk) :: aj(nk),dl(nk),dr(nk)
!!%     REAL(DPK),DIMENSION(ns) :: bcoef
!!%     EXTERNAL interv
!!%     !
!!%     bvalue = 0.0D+00
!!%     IF(x.EQ.t(n+k)) THEN
...
!!!################################################################
    ...
    REAL(dpk) FUNCTION zeta(x, q , l, z, vp)
      ...
      !!$      WRITE(*,*) ' vp   = ', vp
         ...
         omega   =   q**2 - c + 2.0_dpk * zd - vp

         omega1  =  (4.0_dpk/x**2) * ( c + 2.0_dpk * vp  - zd)**2

         omega2  = -(2.0_dpk/x**2) * (3.0_dpk * c + 10.0_dpk * vp - 2.0_dpk *zd)
      
         zeta    = dsqrt(omega) + 0.15625D+00 * omega1 * omega**(-2.5)&
              &    - 0.125_dpk * omega2 * omega**(-1.5)
