...
!  USE ioroutines 
  ...
  !
  REAL(dpk), ALLOCATABLE, DIMENSION(:,:) :: bmx          ! <B_i|x^order|b_j> or <B_i|d/dx|b_j> 
...
!!
  ...
  ALLOCATE( t(1:nb + kb)     )
  call mkgrid(t)                     ! make b-splines gri
  ...
  evaluate_bsplines_matrices:IF( l_a < 0 ) THEN 
     ...
     IF(gauge=='no') THEN 
        ...
        deriv = 0
        order = 0
        CALL bsp_integral(nb, bmx, t, deriv, order)        !<B_i|1|B_j>
        CALL write_mx(-1, bmx, "bb-1")
        WRITE(*,'(a60)'), '<B_i|B_j> evaluated.'
     ENDIF
     ...
     IF(gauge=='l'.OR.gauge=='lva') THEN 
        ...
        CALL bsp_integral(nb, bmx, t, deriv, order)        !<B_i|r|B_j>
     ...
     IF(gauge=='v'.OR.gauge=='lva') THEN
        ...
        deriv = 1
        order = 0
        CALL bsp_integral(nb, bmx, t, deriv,order)      !<B_i|d/dr|B_j>
        CALL write_mx(-1, bmx, "bb-dr")               
        
        bmx = 0.0_dpk
        deriv = 0
        order = -1
        CALL bsp_integral(nb, bmx, t, deriv,order)        !<B_i|1/r|B_j>
     ...
     IF(gauge=='a'.OR.gauge=='lva') THEN
        ...
        deriv = 0
        order = -2
        CALL bsp_integral(nb, bmx, t, deriv,order)         !<B_i|1/r^2|B_j>
  ...
  make_1e_se_on_bsplines: IF(l_a.GE.0) THEN
     ...
     !
     nvalence    = 1                    ! hydrogen 
     CALL setmat(nb, l_a, h, b, t)        ! calculate H0 -E*B
