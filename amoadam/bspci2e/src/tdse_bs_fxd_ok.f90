...
PROGRAM tdse_bs_fxd_2e
  ...
  INTEGER                                :: nout_T 
  !
  REAL(dpk)                              :: t, t1, tim, pop
  REAL(dpk)                              :: pop_g, pop_b, pop_k
  !pes!
  ...
  !
!  INTEGER                                :: ni, nf, lb,lz
  INTEGER i, j, p, q, k, k1, nb, iu, lstart, kt,lk
  ...
  !nag
  EXTERNAL                        d02baf        ! nag
  EXTERNAL                 tdse_v,tdse_v1, tdse_l        ! user defined               
  !exe!
...
!  atomic_file = "dmx2ebb"      
  ...
  IF(hohg) THEN
     form = "l"
     WRITE(*,*) ' form = ', form 
     CALL read_system(form)           ! read en/dmx from f90 bin (dat/systemLL+1l.dat)
!!%!    CALL read_system_ncFile('l')        ! read en/dmx from netCDF (dat/systemLL+1gauge.nc)
!!%
!!%!..... acceleration
!!%    !ALLOCATE(  da( 1:nmax, 1:nmax, 1:lmax ) )
!!%    !CALL read_system('a')        ! read en/dmx from f90 bin (dat/systemLL+1a.dat)
!!%    !CALL read_system_ncFile('a')        ! read en/dmx from netCDF (dat/systemLL+1gauge.nc)
  ENDIF
  ...
  nout_T      = 20 
  t           = 0.0D+00
  CALL save_field (t, nout_T)            !  out/pulse.out
  ...
  change_energy_axis: DO  l = 1, lmax+1
     ...
     exclude_high_energies: DO  ne = 1, n(l)
        ...
        !en(ne,l) = en(ne,l) - en_ion_1                   ! (in a.u.)
...
210  lstart = INT(0.5 * ( t + 0.5 * tau ) /Tp ) + 1
 ...
 IF(hohg) OPEN(ndip, file=dip_file)         ! rt(t) file
...
!  ALLOCATE(work(100+21*neq))         ! work array for ode
  ...
  time_loop:DO  l = lstart, cycles !* tSteps
     ...
     ! break more steps for calculating the time-dependent rt 
     nout1 = kt + 1
...
!                         & + DOT_PRODUCT( y(ntot+nsum(li):ntot+nsum(li) + n(li) - 1 ), dpi)
             ...
             WRITE(nion,'(4E25.14)') tim,  pop_g, pop_b, pop_k 
...
!                WRITE(ndip,'(2x,5e25.12)') time(ikt), at(ikt), vt(ikt),rt(ikt), E_t(time(ikt))
                WRITE(ndip,'(2x,6e25.12)') time(ikt),          at(ikt), & !a(t)
         &                                                     vt(ikt), & !v(t)
         &                                                     rt(ikt), & !r(t) 
         &                                                     A_t(time(ikt)), & !r(t)
         &                                                     E_t(time(ikt))    !e(t)
!            WRITE(ndip,'(2x,5e25.12)') time(ikt), at(ikt), vt(ikt), rt(ikt), E_t(time(ikt))
          ...
          population_t: DO  k = 1, lmax+1
             WRITE(npop,'(i3)') k - 1       ! angular momentum
             

             !  |Y(nL)|^2  , n = 1-5, L = 0, Lmax

             DO k1 = 1, 5      
                WRITE(npop,'(3e15.6)') en(k1,k), ( y(k1+nt)**2 + y(ntot+nt+k1)**2 )
             ENDDO
